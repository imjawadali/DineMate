...
import { GET_TABLE_ORDERS, SET_TOAST, SPLIT_ITEM } from '../../../../../constants'

import OrdersList from './OrdersList'

function ItemSplit(props) {
  ...
  useEffect(() => {
    if (!state)
      ...
      setitemToSplit(state.itemToSplit)
      setitemOrderNumber(state.orderNumber)
  ...
  useEffect(() => {
    ...
    if (splitType === 'equal') {
      for (let index = 0; index < temp.length; index++) {
        temp[index].totalPrice = (itemToSplit.totalPrice / temp.length).toFixed(2)
        const quantity = itemToSplit.quantity * (temp[index].totalPrice / itemToSplit.totalPrice)
  ...
  const onSelect = (orderNumber) => {
    ...
    if (split && split.length) {
      ...
      temp.push({
        restaurantId,
        orderNumber,
        name: `Split - Chk # ${itemOrderNumber} - ${itemToSplit.name}`,
        status: 'R',
        totalPrice: 0,
        quantity: 0,
        splitted: 1
  ...
  const onChange = (orderNumber, price) => {
    ...
    const quantity = itemToSplit.quantity * (price / itemToSplit.totalPrice)
    temp[temp.indexOf(split[0])].quantity = Number.isInteger(quantity) ? quantity : quantity.toFixed(2)
    setsplittedItem(temp)
  }

  const submit = () => {
    if (splittedItemTotal != itemToSplit.totalPrice)
      dispatch(customisedAction(SET_TOAST, { message: 'Splitted sum is not equal to item total', type: 'error' }))
    else
      dispatch(customisedAction(SPLIT_ITEM, { id: itemToSplit.id, splittedItem }, { history }))
  }

  return (
    <div className="Container">
      <TitleWithAction
        text={`Item Splitting (Table - ${tableId && tableId.length === 1 ? 0 : ''}${tableId})`}
        textAlign="center"
        noMargin
        icon={<i
      ...
      <div style={{ height: '95%', display: 'flex', flexDirection: 'row' }}>
        ...
        <div className="TabularContentContainer" style={{ flex: 2 }} >
          <OrdersList fetchingTableOrders={fetchingTableOrders} tableOrders={tableOrders} splittedItem={splittedItem} splitItemId={itemToSplit.id} />
        </div>
      </div>
    </div>
  )
