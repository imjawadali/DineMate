...
import { GET_TABLE_ORDERS, SET_TOAST, SPLIT_ORDER } from '../../../../../constants'

import OrdersList from './OrdersList'

function OrderSplit(props) {

  const [tableId, settableId] = useState(null)
  const [orderToSplit, setorderToSplit] = useState({})
  ...
  useEffect(() => {
    if (!state)
      ...
      setorderToSplit(state.orderToSplit)
  ...
  useEffect(() => {
    ...
    if (splitType === 'equal') {
      for (let index = 0; index < temp.length; index++) {
        temp[index].totalPrice = (orderToSplit.totalPrice / temp.length).toFixed(2)
        const quantity = orderToSplit.quantity * (temp[index].totalPrice / orderToSplit.totalPrice)
  ...
  const onSelect = (orderNumber) => {
    ...
    if (split && split.length) {
      ...
      temp.push({
        restaurantId,
        orderNumber,
        name: `Split - Chk # ${orderToSplit.orderNumber}`,
        status: 'R',
        totalPrice: 0,
        quantity: 0,
        splitted: 1
  ...
  const onChange = (orderNumber, price) => {
    ...
    const quantity = orderToSplit.quantity * (price / orderToSplit.totalPrice)
    temp[temp.indexOf(split[0])].quantity = Number.isInteger(quantity) ? quantity : quantity.toFixed(2)
    setsplittedItem(temp)
  }

  const submit = () => {
    if (splittedItemTotal != orderToSplit.totalPrice)
      dispatch(customisedAction(SET_TOAST, { message: 'Splitted sum is not equal to item total', type: 'error' }))
    else
      dispatch(customisedAction(SPLIT_ORDER, { splitRestaurantId: restaurantId, splitOrderNumber: orderToSplit.orderNumber, splittedItem }, { history }))
  }

  return (
    <div className="Container">
      <TitleWithAction
        text={`Order Splitting (Table - ${tableId && tableId.length === 1 ? 0 : ''}${tableId})`}
        textAlign="center"
        noMargin
        icon={<i
      ...
      <div style={{ height: '95%', display: 'flex', flexDirection: 'row' }}>
            ...
            <div className="TableDataContainer">
              <table>
                <thead>
                  <tr>
                    <th />
                    <th>Split - Chk # {orderToSplit.orderNumber}</th>
                    <th style={{ textAlign: 'center' }}>${(orderToSplit.totalPrice || 0).toFixed(2)}</th>
                  </tr>
                </thead>
                <tbody>
        ...
        <div className="TabularContentContainer" style={{ flex: 2 }} >
          <OrdersList fetchingTableOrders={fetchingTableOrders} tableOrders={tableOrders} splittedItem={splittedItem} splitOrderNumber={orderToSplit.orderNumber} />
        </div>
      </div>
    </div>
  )
