                ...
                <>
                  {
                    orderItems && orderItems.length ?
                      orderItems.map((orderItem, index) => {
                        if (orderNumber !== splitOrderNumber) foodTotal += orderItem.totalPrice
                        ...
                        return (
                          <tr key={orderItem.id} className={index === orderItems.length - 1 ? '' : 'NoBorder'}>
                            ...
                            <td style={{ width: '20vw', color: orderNumber !== splitOrderNumber ? '' : 'lightBlue' }}>{orderItem.name}</td>
                            {<td style={{ textAlign: 'center', color: orderNumber !== splitOrderNumber ? '' : 'lightBlue' }}>{orderNumber !== splitOrderNumber ? orderItem.quantity : 0}</td>}
                            {<td style={{ textAlign: 'end', color: orderNumber !== splitOrderNumber ? '' : 'lightBlue' }}>${(orderNumber !== splitOrderNumber ? orderItem.totalPrice : 0).toFixed(2)}</td>}
                          </tr>
                        )
                      })
                      :
